// ============================================================
// LUMA BROWSER — Main Process (v2.0 — 103 Features)
// ============================================================
...
// Strip Electron/LUMA from the default user agent at the app level
...
// --- Data Storage ---
const DATA_DIR = () => path.join(app.getPath('userData'), 'LUMA-data');
...
function setupAntiFingerprint() {
  ...
  mainWindow.webContents.on('did-attach-webview', (event, wc) => {
    ...
    // so they can access window.LUMA API
    wc.on('will-navigate', (e, url) => {
    ...
    wc.on('dom-ready', () => {
      ...
      if (url.startsWith('file://')) {
        ...
        if (url.includes('youtube.com')) {
          ...
          wc.executeJavaScript(`
            (function() {
              if (window.__LUMAYTAdBlock) return;
              window.__LUMAYTAdBlock = true;
...
ipcMain.handle('page:screenshot', async (e) => {
  try {
    ...
    const savePath = path.join(app.getPath('pictures'), `LUMA-screenshot-${Date.now()}.png`);
...
// BROWSER DATA IMPORT
// ============================================================
const IMPORT_MARKER = path.join(app.getPath('userData'), '.LUMA-imported');
...
ipcMain.handle('import:bookmarks', async (_, browser) => {
  ...
  try {
    ...
    // Save to LUMA bookmarks
    const dataDir = DATA_DIR();
    if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
    const LUMABookmarks = path.join(dataDir, 'bookmarks.json');
    let existing = [];
    if (fs.existsSync(LUMABookmarks)) {
      try { existing = JSON.parse(fs.readFileSync(LUMABookmarks, 'utf-8')); } catch (e) { }
    }
    const merged = [...existing, ...bookmarks];
    fs.writeFileSync(LUMABookmarks, JSON.stringify(merged, null, 2));
...
ipcMain.handle('import:history', async (_, browser) => {
  ...
  try {
    ...
    const tmpPath = path.join(app.getPath('temp'), `LUMA-import-history-${Date.now()}.db`);
    ...
    // Save to LUMA history
    const dataDir = DATA_DIR();
    if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
    const LUMAHistory = path.join(dataDir, 'history.json');
    let existing = [];
    if (fs.existsSync(LUMAHistory)) {
      try { existing = JSON.parse(fs.readFileSync(LUMAHistory, 'utf-8')); } catch (e) { }
    }
    const merged = [...historyItems, ...existing];
    fs.writeFileSync(LUMAHistory, JSON.stringify(merged, null, 2));
...
// ============================================================
app.name = 'LUMA';
app.setAppUserModelId('com.LUMA.browser');
