; (function () {
    ...
    function isUrl(s) {
        return /^(https?:\/\/|file:\/\/|LUMA:\/\/)/.test(s) || /^[\w-]+(\.[\w-]+)+/.test(s) || /^localhost/.test(s);
    }
    ...
    function normalizeUrl(input) {
        const s = input.trim();
        if (!s) return '';
        if (/^(LUMA|https?|file):\/\//.test(s)) return s;
    ...
    function createTab(url = '', opts = {}) {
        // Block creating duplicate LUMA home tabs (unless forced)
        ...
        const tab = {
            id, url: url || '', title: isIncog ? (state.settings.language === 'en' ? 'Incognito' : 'Инкогнито') : (isHomeTab ? 'LUMA' : (opts.title || t('newTab'))),
        ...
        // Add preload for local file:// URLs so they get window.LUMA API
    ...
    function setupWebview(id, wv) {
        ...
        wv.addEventListener('did-navigate', (e) => {
            ...
            if (!t?.incognito && !e.url.includes('newtab.html') && !e.url.includes('incognito.html'))
                window.LUMA.history.add({ url: e.url, title: t?.title, favicon: t?.favicon });
        });
            ...
            wv.executeJavaScript(`
                document.addEventListener('wheel', function(e) {
                    if (e.ctrlKey) {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('__LUMA_ZOOM__:' + (e.deltaY < 0 ? 'in' : 'out'));
                    }
                }, { passive: false, capture: true });
            `).catch(() => { });
        });
        wv.addEventListener('console-message', (e) => {
            if (e.message && e.message.startsWith('__LUMA_ZOOM__:')) {
    ...
    async function renderSidebarPanel(panel) {
        ...
        if (panel === 'bookmarks') {
            const bms = await window.LUMA.bookmarks.get();
            ...
            bms.forEach(b => {
                ...
                it.querySelector('.sidebar-item-delete').addEventListener('click', async () => { await window.LUMA.bookmarks.remove(b.id); renderSidebarPanel('bookmarks'); });
                c.appendChild(it);
            });
        } else if (panel === 'history') {
            const h = await window.LUMA.history.get();
            const lang = state.settings.language || 'ru';
            ...
            sortedKeys.forEach(label => {
                ...
                header.querySelector('.sidebar-clear-day-btn').addEventListener('click', async () => {
                    for (const entry of group.items) {
                        if (entry.id) await window.LUMA.history.remove(entry.id);
                    }
            ...
            d.forEach(dl => {
                ...
                it.addEventListener('click', () => window.LUMA.downloads.showInFolder(dl.path));
                c.appendChild(it);
            });
        } else if (panel === 'notes') {
            ...
            const notes = await window.LUMA.notes.get();
            ...
            notes.forEach(n => {
                ...
                it.querySelector('.sidebar-item-delete').addEventListener('click', async () => { await window.LUMA.notes.delete(n.id); renderSidebarPanel('notes'); });
                nl.appendChild(it);
            });
            c.querySelector('#save-note-btn').addEventListener('click', async () => {
                const txt = c.querySelector('#new-note').value.trim();
                if (txt) { await window.LUMA.notes.save({ text: txt }); renderSidebarPanel('notes'); toast('Заметка сохранена', 'success'); }
            });
        } else if (panel === 'readinglist') {
            const rl = await window.LUMA.readinglist.get();
            ...
            rl.forEach(r => {
                ...
                it.querySelector('.sidebar-item-delete').addEventListener('click', async () => { await window.LUMA.readinglist.remove(r.id); renderSidebarPanel('readinglist'); });
                c.appendChild(it);
            });
    ...
    function showDropdownMenu() {
        const items = [
            ...
            { label: t('menuNewWindow'), icon: ICONS.window, kbd: 'Ctrl+N', action: () => window.LUMA.window.newWindow() },
            ...
            { label: t('menuFullscreen'), icon: ICONS.fullscreen, kbd: 'F11', action: () => window.LUMA.window.fullscreen() },
            { separator: true },
            { label: t('menuSettings'), icon: ICONS.settings, action: () => createTab(settingsUrl(), { title: t('menuSettings') }) },
            { separator: true },
            { label: state.settings.language === 'en' ? 'Quit' : 'Выход', icon: ICONS.exit, action: () => window.LUMA.window.close(), danger: true },
        ];
    ...
    function renderCommands(query) {
        const cmds = [
            ...
            { title: t('menuFullscreen'), kbd: 'F11', action: () => window.LUMA.window.fullscreen() },
    ...
    async function takeScreenshot() {
        const r = await window.LUMA.actions.screenshot();
    ...
    async function showAutocomplete(q) {
        ...
        acTimer = setTimeout(async () => {
            const res = await window.LUMA.history.search(q);
    ...
    async function updatePulse() {
        const s = await window.LUMA.pulse.getStats();
    ...
    function applySettings() {
        ...
        if (typeof s.alwaysOnTop !== 'undefined') {
            window.LUMA.window.alwaysOnTop(!!s.alwaysOnTop);
        }
        // Language
        applyLanguage();
    }
    ...
    window.LUMA.settings.onChanged((newSettings) => {
    ...
    function handleShortcut(key, ctrl, shift) {
        ...
        if (ctrl && !shift && (key === 'n' || key === 'N')) { window.LUMA.window.newWindow(); return true; }
        ...
        if (key === 'F11') { window.LUMA.window.fullscreen(); return true; }
    ...
    // ============================================================
    function bindEvents() {
        dom.btnMinimize.addEventListener('click', () => window.LUMA.window.minimize());
        dom.btnMaximize.addEventListener('click', () => window.LUMA.window.maximize());
        dom.btnClose.addEventListener('click', () => window.LUMA.window.close());
        ...
        dom.urlBookmark.addEventListener('click', async () => {
            ...
            if (t) { await window.LUMA.bookmarks.add({ url: t.url, title: t.title, favicon: t.favicon }); toast('Закладка добавлена', 'success'); dom.urlBookmark.classList.add('bookmarked'); }
        });
        ...
        dom.downloadsOpenFolder.addEventListener('click', () => {
            window.LUMA.actions.openDownloadsFolder?.();
        });
        ...
        // IPC listeners
        window.LUMA.window.onStateChanged((d) => { dom.btnMaximize.title = d.maximized ? 'Восстановить' : 'Развернуть'; });
        window.LUMA.window.onFullscreenChanged((fs) => {
        ...
        window.LUMA.on.openUrl((url) => createTab(url));
        window.LUMA.on.toast((data) => toast(data.message, data.type));

        // Download progress bar + panel
        window.LUMA.downloads.onProgress((d) => {
        ...
        window.LUMA.downloads.onComplete((d) => {
            dom.downloadBarName.textContent = d.filename;
    ...
    async function toggleAiVoiceAgent() {
        ...
        try {
            ...
            const provider = await window.LUMA.ai.getProvider();
    ...
    async function connectOpenAIVoice() {
        ...
        const tokenResult = await window.LUMA.ai.getRealtimeToken();
...
6. НИКОГДА не говори "я использую инструмент" — ты ЧЕЛОВЕК`,
            ...
            try {
                ...
                if (msg.type === 'response.function_call_arguments.done') {
                    ...
                    if (toolName === 'open_website') {
                        ...
                        await window.LUMA.ai.executeAction({ type: 'navigate', url: toolArgs.url });
                    }
                        ...
                        await window.LUMA.ai.executeAction({ type: 'navigate', url: url });
                    }
                        ...
                        const jsCode = `
                            ...
                            `;
                        const r = await window.LUMA.ai.executeAction({ type: 'executeJS', code: jsCode });
                        ...
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `window.scrollBy({top:${px},behavior:'smooth'})` });
                    }
                    else if (toolName === 'go_back') {
                        dom.aiOrbStatus.textContent = "Назад...";
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `history.back()` });
                    }
                    else if (toolName === 'go_forward') {
                        dom.aiOrbStatus.textContent = "Вперёд...";
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `history.forward()` });
                    }
                    else if (toolName === 'describe_page') {
                        dom.aiOrbStatus.textContent = "Смотрю...";
                        try {
                            const screenshot = await window.LUMA.ai.captureTab();
                            if (screenshot && screenshot.base64) {
                                // Send screenshot to GPT-4o REST API for actual vision analysis
                                const visionResult = await window.LUMA.ai.describeScreen(screenshot.base64);
        ...
        const sdpResult = await window.LUMA.ai.sendSDP(EPHEMERAL_KEY, offer.sdp);
        if (sdpResult.error) {
    ...
    async function connectGeminiVoice() {
        ...
        const result = await window.LUMA.ai.connectGemini();
        if (result.error) {
        ...
        processor.onaudioprocess = (e) => {
            ...
            window.LUMA.ai.sendGeminiAudio(base64);
        };
        ...
        window.LUMA.ai.onGeminiEvent(async (msg) => {
            if (!aiState.isActive) return;
            ...
            if (msg.toolCall && msg.toolCall.functionCalls) {
                for (const fc of msg.toolCall.functionCalls) {
                    ...
                    if (toolName === 'open_website') {
                        ...
                        await window.LUMA.ai.executeAction({ type: 'navigate', url: `https://www.google.com/search?q=${encodeURIComponent(toolArgs.query)}` });
                    } else if (toolName === 'search_youtube') {
                        dom.aiOrbStatus.textContent = "YouTube...";
                        await window.LUMA.ai.executeAction({ type: 'navigate', url: `https://www.youtube.com/results?search_query=${encodeURIComponent(toolArgs.query)}` });
                    } else if (toolName === 'click_text') {
                        dom.aiOrbStatus.textContent = "Кликаю...";
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `(function(){const e=document.evaluate("//*[contains(text(),'${toolArgs.text.replace(/'/g, "\\'")}')]",document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null);if(e.singleNodeValue)e.singleNodeValue.click();})()` });
                    } else if (toolName === 'type_text') {
                        dom.aiOrbStatus.textContent = "Печатаю...";
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `(function(){const i=document.querySelector('input[placeholder*="${toolArgs.target_text}"],textarea[placeholder*="${toolArgs.target_text}"]');if(i){i.focus();i.value='${toolArgs.value.replace(/'/g, "\\'")}';i.dispatchEvent(new Event('input',{bubbles:true}))}})()` });
                    } else if (toolName === 'scroll') {
                        dom.aiOrbStatus.textContent = "Скролл...";
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `window.scrollBy(0, ${toolArgs.direction === 'down' ? 500 : -500})` });
                    } else if (toolName === 'go_back') {
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `history.back()` });
                    } else if (toolName === 'go_forward') {
                        await window.LUMA.ai.executeAction({ type: 'executeJS', code: `history.forward()` });
                        ...
                        try {
                            ...
                            if (screenshot && screenshot.base64) {
                                ...
                                await window.LUMA.ai.sendGeminiImage(screenshot.base64);
                    ...
                    // Send tool response
                    await window.LUMA.ai.sendGeminiToolResponse([{
    ...
    function closeAiVoiceAgent() {
        ...
        if (aiState._provider === 'gemini') {
            window.LUMA.ai.disconnectGemini();
            window.LUMA.ai.removeGeminiListeners();
        }
    ...
    // ============================================================
    async function init() {
        state.settings = await window.LUMA.settings.load();
        try { _preloadPath = await window.LUMA.app.getPreloadPath(); } catch (e) { }
        applySettings();
        bindEvents();
        await initIntro();
        createTab();
